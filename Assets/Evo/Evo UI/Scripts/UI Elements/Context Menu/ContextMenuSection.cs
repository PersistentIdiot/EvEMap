...
namespace Evo.UI
{
    ...
    public class ContextMenuSection : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerClickHandler
    {
        ...
        void PositionSubmenu()
        {
            ...
            // Get section's world corners in screen space
            Vector3[] sectionCorners = new Vector3[4];
            sectionRect.GetWorldCorners(sectionCorners);
            // 0 = bottom-left, 1 = top-left, 2 = top-right, 3 = bottom-right

            // Get submenu dimensions
            float submenuWidth = submenuRect.rect.width;

            // Calculate position starting from top-right corner of section
            Vector3 targetPos = sectionCorners[2]; // Top-right corner
            ...
            if (targetPos.x + submenuWidth > Screen.width - screenPadding)
            {
                ...
                targetPos = sectionCorners[1]; // Top-left corner
                targetPos.x -= submenuOffset.x;
                targetPos.y += submenuOffset.y;
                submenuOnLeft = true;
            }
            ...
            // Set position
            submenuRect.position = targetPos;
        }
        ...
        {
            float width = submenuRect.rect.width;
            float height = submenuRect.rect.height;
            float padding = 10f;
