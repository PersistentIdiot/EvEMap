...
namespace Evo.UI
{
    ...
    public class ContextMenu : MonoBehaviour, IPointerClickHandler
    {
        ...
        void PositionMenu()
        {
            ...
            // Calculate position
            targetPosition = CalculateMenuPosition();

            // Set position directly for non-slide animations
            if (animationType != AnimationType.Slide) { menuRect.position = targetPosition; }
        }

        Vector3 CalculateMenuPosition()
        {
            if (menuInstance == null) { return Vector3.zero; }
            if (!menuInstance.TryGetComponent<RectTransform>(out var menuRect)) { return Vector3.zero; }
            ...
            else
            {
                ...
                {
                    // Fallback to last click position
                    basePosition = lastClickPosition;
                }
            }

            Vector3 position = basePosition + GetOffsetVector();
            return ClampToScreen(position, menuRect);
        }
