    ...
    public class Tabs : MonoBehaviour
    {
        ...
        void InitializeTabs()
        {
            ...
            for (int i = 0; i < tabs.Count; i++)
            {
                ...
                {
                    if (tabIDToIndex.ContainsKey(tab.tabID)) { Debug.LogWarning($"Duplicate tab ID '{tab.tabID}' found at index {i}. " +
        ...
        public void OpenTab(int tabIndex)
        {
            ...
            if (gameObject.activeInHierarchy) { StartCoroutine(SwitchTab(tabIndex)); }
            else 
            {
