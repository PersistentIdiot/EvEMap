...
namespace Evo.UI.Tools
{
    ...
    public class StylerPresetBrowser : EditorWindow, IEvoEditorGUIHandler
    {
        ...
        void UpdateDetailsPanel()
        {
            ...
            StylerObject stylerObj = activeObj != null ? activeObj.GetComponent<StylerObject>() : null;
            if (stylerObj != null)
            {
                var applyBtn = CreateIconButton($"Apply to '{activeObj.name}'", "Dependency@2x", () => ApplyPresetToSelection(stylerObj));
                applyBtn.style.height = ACTION_ITEM_SIZE;
                applyBtn.style.marginTop = ACTION_SPACING;
                headerContainer.Add(applyBtn);
            }
        ...
        void ApplyPresetToSelection(StylerObject styler)
        {
            if (styler == null || selectedPreset == null)
                return;

            Undo.RecordObject(styler, "Apply Styler Preset");
            styler.preset = selectedPreset;
            styler.UpdateStyle();
